...
TMP="/tmp/build_52nSOS_FOSS4G2016"
if [ -z "$USER_NAME" ] ; then
   USER_NAME="user"
fi
...
SOS_WEB_APP_NAME="52nSOS-FOSS4G2016"
...
SOS_INSTALL_FOLDER="/usr/local/52nSOS-FOSS4G2016"
SOS_BIN_FOLDER="/usr/local/share/52nSOS-FOSS4G2016"
SOS_TAR_NAME="52N-SOS-FOSS4G2016.tar.gz"
SOS_TAR_URL="./"
SOS_VERSION="4.4.0"
PG_OPTIONS='--client-min-messages=warning'
PG_USER="postgres"
PG_SCRIPT_NAME="postgresql"
PG_DB_NAME="52nSOS-FOSS4G2016"
...
cp -v "$BUILD_DIR/$SOS_TAR_NAME" "$TMP"/
#
...
su $PG_USER -c "PGOPTIONS='$PG_OPTIONS' psql $PG_DB_NAME -c 'create schema postgis;'"
su $PG_USER -c "PGOPTIONS='$PG_OPTIONS' psql $PG_DB_NAME -c 'create extension postgis SCHEMA postgis;'"
...
#
su $PG_USER -c "PGOPTIONS='$PG_OPTIONS' psql -d $PG_DB_NAME -q -f $TMP/schema_rawdata.sql"
su $PG_USER -c "PGOPTIONS='$PG_OPTIONS' psql -d $PG_DB_NAME -q -f $TMP/schema_sos_rawdb.sql"
...
# Enabled autoDeploy
#
egrep -rn "autoDeploy|unpackWARs" "/etc/$TOMCAT_SCRIPT_NAME/server.xml"
sed -i -e 's/autoDeploy="false"/autoDeploy="true"/g' "/etc/$TOMCAT_SCRIPT_NAME/server.xml"
sed -i -e 's/unpackWARs="false"/unpackWARs="true"/g' "/etc/$TOMCAT_SCRIPT_NAME/server.xml"
egrep -rn "autoDeploy|unpackWARs" "/etc/$TOMCAT_SCRIPT_NAME/server.xml"
...
service $TOMCAT_SCRIPT_NAME start
#
#
#
...
if [ ! -e $SOS_BIN_FOLDER/52nSOS-FOSS4G-start.sh ] ; then
   cat << EOF > $SOS_BIN_FOLDER/52nSOS-FOSS4G-start.sh
#!/bin/bash
...
#
if [ ! -e $SOS_BIN_FOLDER/52nSOS-FOSS4G-stop.sh ] ; then
   cat << EOF > $SOS_BIN_FOLDER/52nSOS-FOSS4G-stop.sh
#!/bin/bash
TOMCAT=\`sudo service $TOMCAT_SCRIPT_NAME status | grep pid | wc -l\`
if [ \$TOMCAT -eq 1 ]; then
...
#
chmod 755 $SOS_BIN_FOLDER/52nSOS-FOSS4G-start.sh
chmod 755 $SOS_BIN_FOLDER/52nSOS-FOSS4G-stop.sh
#
#
# Desktop set-up
# =============================================================================
...
if [ ! -e /usr/local/share/applications/52nSOS-FOSS4G-start.desktop ] ; then
   cat << EOF > /usr/local/share/applications/52nSOS-FOSS4G-start.desktop
[Desktop Entry]
Type=Application
Encoding=UTF-8
Name=Start 52NorthSOS FOSS4G2016
Comment=52North SOS FOSS4G2016
Categories=Geospatial;Servers;
Exec=$SOS_BIN_FOLDER/52nSOS-FOSS4G-start.sh
...
#
#
cp -v /usr/local/share/applications/52nSOS-FOSS4G-start.desktop "$USER_HOME/Desktop/"
chown -v $USER_NAME:$USER_NAME "$USER_HOME/Desktop/52nSOS-FOSS4G-start.desktop"
#
if [ ! -e /usr/local/share/applications/52nSOS-FOSS4G-stop.desktop ] ; then
   cat << EOF > /usr/local/share/applications/52nSOS-FOSS4G-stop.desktop
[Desktop Entry]
Type=Application
Encoding=UTF-8
Name=Stop 52NorthSOS FOSS4G2016
Comment=52North SOS FOSS4G2016
Categories=Geospatial;Servers;
Exec=$SOS_BIN_FOLDER/52nSOS-FOSS4G-stop.sh
Icon=/usr/local/share/icons/$SOS_ICON_NAME
Terminal=false
EOF
fi
#
cp -v /usr/local/share/applications/52nSOS-FOSS4G-stop.desktop "$USER_HOME/Desktop/"
chown -v $USER_NAME:$USER_NAME "$USER_HOME/Desktop/52nSOS-FOSS4G-stop.desktop"
#
#
